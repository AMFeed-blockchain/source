...
// which are part of the AMFeed repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="amfeed-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /amfeed-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "amfeedpay-amfeed/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from amfeedpay-amfeed/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'amfeedpay-amfeed/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from amfeedpay-amfeed/develop"
            }
          }

          if (hasCache) {
            sh "cd /amfeed-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /amfeed-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /amfeed-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /amfeed-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /amfeed-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /amfeed-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /amfeed-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
