...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all AMFeed related functionality
...
/*
    ...
    allow endless transaction that would bloat AMFeed and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all AMFeed specific functionality
