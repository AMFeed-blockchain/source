...
const char * const BITCOIN_CONF_FILENAME = "amfeed.conf";
const char * const BITCOIN_PID_FILENAME = "amfeedd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "amfeed" is a composite category enabling all AMFeed-related debug output
                if(ptrCategory->count(std::string("amfeed"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AMFeed
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AMFeed
    // Mac: ~/Library/Application Support/AMFeed
    // Unix: ~/.amfeed
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AMFeed";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AMFeed";
#else
    // Unix
    return pathRet / ".amfeed";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty amfeed.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override amfeed.conf
