...
pkgdatadir = $(datadir)/amfeedcore
pkgincludedir = $(includedir)/amfeedcore
pkglibdir = $(libdir)/amfeedcore
pkglibexecdir = $(libexecdir)/amfeedcore
...
CONFIG_HEADER = $(top_builddir)/src/config/amfeed-config.h
CONFIG_CLEAN_FILES = libamfeedconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libamfeedconsensus.pc.in \
	...
	$(top_srcdir)/src/config/amfeed-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/AMFeed/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/AMFeed/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/AMFeed/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/AMFeed/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = amfeed-cli
BITCOIN_DAEMON_NAME = amfeedd
BITCOIN_GUI_NAME = amfeed-qt
BITCOIN_TX_NAME = amfeed-tx
...
COPYRIGHT_HOLDERS_FINAL = The AMFeed Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = AMFeed Core
COPYRIGHT_YEAR = 2019
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/AMFeed/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = amfeedcore
PACKAGE_BUGREPORT = https://github.com/amfeedpay/amfeed/issues
PACKAGE_NAME = AMFeed Core
PACKAGE_STRING = AMFeed Core 1.0.0
PACKAGE_TARNAME = amfeedcore
PACKAGE_URL = https://amfeed.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/AMFeed
abs_srcdir = /tmp/AMFeed
abs_top_builddir = /tmp/AMFeed
abs_top_srcdir = /tmp/AMFeed
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/AMFeed/build-aux/install-sh
...
pkgconfig_DATA = libamfeedconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/amfeed-cli.bash-completion \
	       $(top_srcdir)/contrib/amfeed-tx.bash-completion \
	       $(top_srcdir)/contrib/amfeedd.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_amfeed_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_amfeed_coverage.info test_amfeed.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/AMFeed-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/amfeed-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/amfeed-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/amfeed-config.h
$(top_srcdir)/src/config/amfeed-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/amfeed-config.h src/config/stamp-h1
libamfeedconsensus.pc: $(top_builddir)/config.status $(srcdir)/libamfeedconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/AMFeed-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/AMFeed-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/AMFeed-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_amfeed.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_amfeed -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_amfeed_filtered.info: test_amfeed.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_amfeed_filtered.info
...
#test_amfeed_coverage.info: baseline_filtered_combined.info test_amfeed_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_amfeed_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_amfeed_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_amfeed_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_amfeed.coverage/.dirstamp:  test_amfeed_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_amfeed.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_amfeed.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
