...
# amfeed_hash
RUN git clone https://github.com/amfeedpay/amfeed_hash
RUN cd amfeed_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} amfeed
RUN useradd -u ${USER_ID} -g amfeed -s /bin/bash -m -d /amfeed amfeed
...
RUN mkdir /amfeed-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /amfeed-src && \
